...
**Ядро аналитики и NL→SQL на уровне API и данных:** схемы БД по именованным источникам, интерпретация вопросов, история чатов, политики доступа, внутренний sync для оркестратора. Сообщение в очередь **`nl_sql_generate_request`** публикует **nl-orchestrator-worker**; выполнение SQL — в **sql-generator-worker** (см. upstream [README](../../analytics-service/README.md)).
