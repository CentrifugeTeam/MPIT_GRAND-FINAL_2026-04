...
WebSocket — документация (клиент)

**NL-чат через BFF** использует **плоский JSON**: поле `type` и остальные поля на **корне** объекта (как `wsClient.sendPlain` в `shared/api/ws-client.ts`), а не вложенный `payload`, как в примерах ниже в этом файле. Типы событий, удаление сообщений, `join_chat` и комнаты `chat:{uuid}` описаны в [nl-chat-and-reports.md](nl-chat-and-reports.md).
