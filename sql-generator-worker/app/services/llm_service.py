...
- Если вопрос — «все данные / вся таблица / выгрузи всё» по одной таблице из схемы: сделай SELECT всех колонок из этой таблицы с ORDER BY по первичному/уникальному полю из схемы или по первой колонке, и обязательно добавь LIMIT (например 5000 или меньше, если вопрос явно просит меньше). Не отказывайся генерировать SQL; не используй SELECT * без LIMIT на больших таблицах.
