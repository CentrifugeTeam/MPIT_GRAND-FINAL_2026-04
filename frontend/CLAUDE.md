...
src/
  ...
  shared/                 # Переиспользуемое без бизнес-логики
    ui/
      atoms/              # Базовые примитивы
      molecules/          # Композиции контролов
      organisms/          # Крупные независимые UI-блоки
      templates/          # Каркасы страниц без бизнес-логики
    lib/                  # Утилиты, хелперы
...
- **Коннекторы** (HTTP/WS-вызовы) живут в `*/api/` соответствующего slice.
- **Адаптеры** (`dto -> domain`) живут в `entities/*/model` или `entities/*/lib`.
- UI и widgets работают с доменными моделями, а не с сырыми DTO.
